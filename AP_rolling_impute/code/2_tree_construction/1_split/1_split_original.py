...
def save_tree_outputs(tree_dir, ret_df, feature_minmax, split_features, leaf_membership_df):
    ...
    ret_df = ret_df.copy()
    ret_num_cols = ret_df.select_dtypes(include=["number"]).columns
    ret_df[ret_num_cols] = ret_df[ret_num_cols].astype("float32")

    ret_df.to_parquet(
        os.path.join(tree_dir, "ret.parquet"),
        compression="zstd",
        index=False
    )

    # Save each min/max object separately, based on its actual type
    for f in split_features:
        short_name = feature_short_name(f)

        for suffix in ["min", "max"]:
            key = f"{f}_{suffix}"
            obj = feature_minmax[key]
            ...
            if isinstance(obj, pd.DataFrame):
                ...
                obj.to_parquet(
                    os.path.join(tree_dir, f"{short_name}_{suffix}.parquet"),
                    compression="zstd",
                    index=False
                )
                ...
                obj.to_frame(name=key).to_parquet(
                    os.path.join(tree_dir, f"{short_name}_{suffix}.parquet"),
                    compression="zstd",
                    index=True
                )
                ...
                # scalar fallback
                pd.DataFrame([{key: obj}]).to_parquet(
                    os.path.join(tree_dir, f"{short_name}_{suffix}.parquet"),
                    compression="zstd",
                    index=False
                )

    # leaf_membership_df: cast only numeric columns
    leaf_membership_df = leaf_membership_df.copy()
    leaf_num_cols = leaf_membership_df.select_dtypes(include=["number"]).columns
    for col in leaf_num_cols:
        if pd.api.types.is_integer_dtype(leaf_membership_df[col]):
            leaf_membership_df[col] = leaf_membership_df[col].astype("int32")
        else:
            leaf_membership_df[col] = leaf_membership_df[col].astype("float32")
