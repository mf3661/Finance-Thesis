...
def concat_yearly_combo_ret(file_paths):
    dfs = []
    ...
    dedup_keys = [c for c in [DATE_COL, TREE_ID_COL, COMBO_ID_COL] if c in out.columns]
    if dedup_keys:
        out = out.drop_duplicates(subset=dedup_keys, keep="last")

    sort_keys = [c for c in [DATE_COL, TREE_ID_COL] if c in out.columns]
    if sort_keys:
...
def reshape_combo_ret_to_tree_blocks(combo_ret_df: pd.DataFrame, combo_id: str):
    """
    New input structure:
    one file per combo per year, with columns:
    - date
    - tree_id
    - combo_id
    - node return columns such as 1, 11, 12, ...

    This function converts that file into one block per tree,
    matching the old combine logic.
    """
    required_cols = {DATE_COL, TREE_ID_COL}
    missing = required_cols - set(combo_ret_df.columns)
    if missing:
        raise ValueError(f"Missing required columns in combo return file for {combo_id}: {missing}")

    node_cols = [c for c in combo_ret_df.columns if c not in [DATE_COL, TREE_ID_COL, COMBO_ID_COL]]
    if len(node_cols) == 0:
        return []

    blocks = []
    for tree_id, tree_df in combo_ret_df.groupby(TREE_ID_COL):
        keep_cols = [DATE_COL] + node_cols
        tree_block = tree_df[keep_cols].copy()
        tree_block = tree_block.drop_duplicates(subset=[DATE_COL]).sort_values(DATE_COL).reset_index(drop=True)
        tree_block = add_prefix(tree_block, combo_id=combo_id, tree_id=str(tree_id))
        tree_block = tree_block.set_index(DATE_COL)
...
def combine_one_repeat(split_input_dir: str, output_dir: str, drop_missing_months: bool):
    ...
    for _, combo_row in combo_specs.iterrows():
        ...
        file_paths = [
            os.path.join(split_input_dir, str(year), f"{combo_id}_ret.parquet")
            for year in years
        ]

        combo_ret_df = concat_yearly_combo_ret(file_paths)

        if combo_ret_df is None:
            print(f"[WARNING] No return files found for {combo_id}", flush=True)
            continue

        tree_blocks = reshape_combo_ret_to_tree_blocks(combo_ret_df, combo_id=combo_id)
        ret_dfs.extend(tree_blocks)
