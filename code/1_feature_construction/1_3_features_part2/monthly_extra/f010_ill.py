...
def load_ill_daily_data(
    conn: Any,
    start_date: str = "01/01/1959",
) -> pd.DataFrame:
    """
    Load daily CRSP return, volume, price, and delisting return data for illiquidity.
    """
    ...
    delisting_df: pd.DataFrame = conn.raw_sql(
        """
        select
            permno,
            dlret,
    ...
    delisting_df["dlstdt"] = pd.to_datetime(delisting_df["dlstdt"])
    delisting_df["date"] = delisting_df["dlstdt"]
    delisting_df["dlret"] = pd.to_numeric(
        delisting_df["dlret"],
        errors="coerce",
    )
    ...
    daily_df = pd.merge(
        daily_df,
        delisting_df[["permno", "date", "dlret"]],
        how="left",
        on=["permno", "date"],
    ).reset_index(drop=True)

    daily_df["dlret"] = daily_df["dlret"].fillna(0)
...
def prepare_monthly_sequence(
    ...
    if missing_cols:
        raise KeyError(
            f"Missing required columns for ill monthly sequence: {missing_cols}"
        )
...
def _estimate_ill_for_window(
    window_df: pd.DataFrame,
    min_obs: int,
) -> float | None:
    """
    Estimate Amihud-type illiquidity from one rolling daily window.
    """
    ...
    ill_series = (
        window_df["retadj"].abs()
        / (window_df["prc"].abs() * window_df["vol"])
    )

    ill_series = ill_series.replace(
        [np.inf, -np.inf],
        np.nan,
    )
...
def _compute_ill_for_permnos(
    args: tuple[pd.DataFrame, list[int], int],
) -> pd.DataFrame:
    """
    Compute ill for one chunk of permnos.
    """
...
def add_ill(
    ...
    """
    Compute rolling three-month daily illiquidity variable.
    """
    ...
    if missing_cols:
        raise KeyError(
            f"Missing required columns for ill calculation: {missing_cols}"
        )
    ...
    if n_processes <= 1:
        ill_df = _compute_ill_for_permnos(
            (
                stock_data_df,
                permno_list,
                min_obs,
    ...
    ill_df["permno"] = ill_df["permno"].astype(int)
    ill_df["date"] = pd.to_datetime(ill_df["date"])
    ill_df["jdate"] = pd.to_datetime(ill_df["jdate"])
