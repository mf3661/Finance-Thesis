...
def load_maxret_daily_data(
    conn: Any,
    start_date: str = "01/01/1959",
) -> pd.DataFrame:
    """
    Load daily CRSP return and volume data for maxret.
    """
...
def prepare_monthly_sequence(
    ...
    if missing_cols:
        raise KeyError(
            f"Missing required columns for maxret monthly sequence: {missing_cols}"
        )
...
def _estimate_maxret_for_window(
    window_df: pd.DataFrame,
    min_obs: int,
) -> float | None:
    """
    Estimate maxret from one rolling daily window.
    """
...
def _compute_maxret_for_permnos(
    args: tuple[pd.DataFrame, list[int], int],
) -> pd.DataFrame:
    """
    Compute maxret for one chunk of permnos.
    """
    ...
    for permno in permno_list:
        ...
        for month_idx in valid_months:
            ...
            maxret = _estimate_maxret_for_window(
                window_df=window_df,
                min_obs=min_obs,
            )
...
def add_maxret(
    ...
    """
    Compute rolling three-month maximum daily return.
    """
    ...
    if missing_cols:
        raise KeyError(
            f"Missing required columns for maxret calculation: {missing_cols}"
        )
    ...
    if n_processes <= 1:
        maxret_df = _compute_maxret_for_permnos(
            (
                stock_data_df,
                permno_list,
                min_obs,
    ...
    maxret_df = maxret_df.dropna(
        subset=["maxret"]
    ).reset_index(drop=True)

    maxret_df["permno"] = maxret_df["permno"].astype(int)
    maxret_df["date"] = pd.to_datetime(maxret_df["date"])
    maxret_df["jdate"] = pd.to_datetime(maxret_df["jdate"])
