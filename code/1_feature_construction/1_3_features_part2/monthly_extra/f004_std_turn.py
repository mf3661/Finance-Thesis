...
def load_std_turn_daily_data(
    conn: Any,
    start_date: str = "01/01/1959",
) -> pd.DataFrame:
    """
    Load daily CRSP volume and shares outstanding data for std_turn.
    """
...
def prepare_monthly_sequence(
    ...
    if missing_cols:
        raise KeyError(
            f"Missing required columns for std_turn monthly sequence: {missing_cols}"
        )
...
def _estimate_std_turn_for_window(
    window_df: pd.DataFrame,
    min_obs: int,
) -> float | None:
    """
    Estimate std_turn from one rolling daily window.
    """
    ...
    daily_turnover = window_df["vol"] / window_df["shrout"]

    std_turn = daily_turnover.std()

    if pd.isna(std_turn):
        return None

    return float(std_turn)


def _compute_std_turn_for_permnos(
    args: tuple[pd.DataFrame, list[int], int],
) -> pd.DataFrame:
    """
    Compute std_turn for one chunk of permnos.
    """
    ...
    for permno in permno_list:
        ...
        for month_idx in valid_months:
            ...
            std_turn = _estimate_std_turn_for_window(
                window_df=window_df,
                min_obs=min_obs,
            )
...
def add_std_turn(
    ...
    """
    Compute rolling three-month daily turnover volatility.
    """
    ...
    if missing_cols:
        raise KeyError(
            f"Missing required columns for std_turn calculation: {missing_cols}"
        )
    ...
    if n_processes <= 1:
        std_turn_df = _compute_std_turn_for_permnos(
            (
                stock_data_df,
                permno_list,
                min_obs,
        ...
        with Pool(processes=n_chunks) as pool:
            result_list = pool.map(
                _compute_std_turn_for_permnos,
                pool_args,
            )

        std_turn_df = pd.concat(
            result_list,
            axis=0,
            ignore_index=True,
        )

    std_turn_df = std_turn_df.dropna(
        subset=["std_turn"]
    ).reset_index(drop=True)

    std_turn_df["permno"] = std_turn_df["permno"].astype(int)
    std_turn_df["date"] = pd.to_datetime(std_turn_df["date"])
    std_turn_df["jdate"] = pd.to_datetime(std_turn_df["jdate"])

    std_turn_df = std_turn_df[
        [
            "permno",
            "date",
            "jdate",
