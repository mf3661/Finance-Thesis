...
def add_sgrvol(df: pd.DataFrame) -> pd.DataFrame:
    """
    Add quarterly revenue surprise volatility variable.
    """
    ...
    if missing_cols:
        raise KeyError(
            f"Missing required columns for quarterly sgrvol: {missing_cols}"
        )

    output_df["sgrvol"] = _compute_chars_std(
        start_lag=0,
        end_lag=15,
        df=output_df,
        characteristic_name="rsup",
    )
