...
CHARS_SUMMARY_PATH: Path = Path(
    "../../../data/common/chars_summary.csv"
)

RANKED_DATA_NO_IMPUTE_OUTPUT_PATH: Path = OUTPUT_DIR / "ranked_data_no_impute.pkl.gz"
RANKED_DATA_IMPUTED_OUTPUT_PATH: Path = OUTPUT_DIR / "ranked_data_imputed.pkl.gz"

RANKED_BY_YEAR_NO_IMPUTE_OUTPUT_DIR: Path = OUTPUT_DIR / "by_year" / "no_impute"
RANKED_BY_YEAR_IMPUTED_OUTPUT_DIR: Path = OUTPUT_DIR / "by_year" / "imputed"
...
def build_ranked_data_for_downstream_pipeline(
    raw_df: pd.DataFrame,
    version_name: str,
    chars_summary_path: Path,
    ranked_data_output_path: Path,
    by_year_output_dir: Path,
) -> pd.DataFrame:
    """
    Build downstream ranked_data pickle and yearly parquet files.
    ...
    if missing_cols:
        raise ValueError(
            f"Missing columns in {version_name} data for downstream ranking: {missing_cols}"
        )
    ...
    print(f"SAVED {version_name}: {ranked_data_output_path}")
    ...
    for year in years:
        ...
        print(f"Saved {version_name} {year}: {df_year.shape}")

    print(f"Saved downstream ranked data successfully: {version_name}")
    print(f"Final downstream ranked shape ({version_name}): {df.shape}")
    print(f"Number of rank features ({version_name}): {len(rank_features)}")
...
def generate_rank_features() -> tuple[
    ...
    downstream_ranked_no_impute_df: pd.DataFrame = build_ranked_data_for_downstream_pipeline(
        raw_df=chars_raw_no_impute,
        version_name="no_impute",
        chars_summary_path=CHARS_SUMMARY_PATH,
        ranked_data_output_path=RANKED_DATA_NO_IMPUTE_OUTPUT_PATH,
        by_year_output_dir=RANKED_BY_YEAR_NO_IMPUTE_OUTPUT_DIR,
    )

    downstream_ranked_imputed_df: pd.DataFrame = build_ranked_data_for_downstream_pipeline(
        raw_df=chars_raw_imputed,
        version_name="imputed",
        chars_summary_path=CHARS_SUMMARY_PATH,
        ranked_data_output_path=RANKED_DATA_IMPUTED_OUTPUT_PATH,
        by_year_output_dir=RANKED_BY_YEAR_IMPUTED_OUTPUT_DIR,
    )
    ...
    return (
        ...
        chars_rank_no_impute,
        chars_rank_imputed,
        downstream_ranked_no_impute_df,
        downstream_ranked_imputed_df,
    )
...
def main() -> None:
    ...
    print(f"downstream no-impute shape:      {downstream_ranked_no_impute_df.shape}")
    print(f"downstream imputed shape:        {downstream_ranked_imputed_df.shape}")
